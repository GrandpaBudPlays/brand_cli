...
class DraftWorkflow(Workflow):
    ...
    def execute(self, context: WorkflowContext, model: GeminiModel) -> None:
        ...
        if pass_number == "1":
            self._run_extraction_pass(context, model)
        elif pass_number == "2":
            self._run_creative_and_seo_pipeline(context, model)
        else:
            print(f"Unknown pass number: {pass_number}")

    # --- Pipeline Coordination ---

    def _run_extraction_pass(self, context: WorkflowContext, model: GeminiModel) -> None:
    ...
    def _run_creative_and_seo_pipeline(self, context: WorkflowContext, model: GeminiModel) -> None:
